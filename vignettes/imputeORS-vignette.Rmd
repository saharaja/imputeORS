...
Simulated data is drawn from a $\beta$ distribution using, for all known observations, the `value` field ($\mu$) for the mean and the `std.error` field ($\sigma_e$) as the standard deviation. The details of this calculation can be found [...](). As a result of this step, each known observation has, in addition to its original value, 10 simulated values associated with it, resulting in 10 simulated datasets.
...
This measure can be used in a correlation calculation across occupations (see \@ref(fig:...), below). Further, it can be used to assess job similarity between different occupations (see \@ref(standardized-ele) for more details). ELE can also be applied to improving survey design, but this is beyond the scope of this vignette (see [...]() for full details).
...
```{r overlap-omega1, echo=FALSE,fig.align='center',out.width="75%",fig.height=3.5,fig.cap="Overlap of Accountants and Web Developers. Mean +/- SD (dashed lines) assumes equal weighting of captured activity."}
include_graphics("overlap_Accountants-v-Web Developers.png")
```

```{r overlap-omega2, echo=FALSE,fig.align='center',out.width="75%",fig.height=3.5,fig.cap="Overlap of Accountants and Actuaries. Mean +/- SD (dashed lines) assumes equal weighting of captured activity."}
include_graphics("overlap_Accountants-v-Actuaries.png")
```

```{r overlap-omega3, echo=FALSE,fig.align='center',out.width="75%",fig.height=3.5,fig.cap="Overlap of Accountants and Chemists. Mean +/- SD (dashed lines) assumes equal weighting of captured activity."}
...
```{r overlap-ele1, echo=FALSE,fig.align='center',out.width="75%",fig.height=3.5,fig.cap="Difference in standardized ELEs between Accountants and Web Developers."}
include_graphics("stdEVdiff_Accountants-v-Web Developers.png")
```

```{r overlap-ele2, echo=FALSE,fig.align='center',out.width="75%",fig.height=3.5,fig.cap="Difference in standardized ELEs between Accountants and Actuaries."}
include_graphics("stdEVdiff_Accountants-v-Actuaries.png")
```

```{r overlap-ele3, echo=FALSE,fig.align='center',out.width="75%",fig.height=3.5,fig.cap="Difference in standardized ELEs between Accountants and Chemists."}
